...
const SCF = {
  ...
  "getSearchKeyword": function () {
    return SCF.isEmpty($("#searchKeyword").val()) ? "" : $(
...
$(function () {
  ...
  // URL 쿼리 파라미터에서 'keyword' 값을 읽어와 검색창에 설정
  ...
  $(document).on('click', function (e) {
    if (e.target.id != "searchKeyword" && !$(e.target).hasClass(
        "auto_complete_maintain") && $(e.target).parents(
        '.auto_complete_maintain').length <= 0) {
...
function setAutoCompleteCookie(name, value, expire) {
  ...
  document.cookie = name + "=" + escape(value) + "; path=/; expires="
      + expire_date.toUTCString() + "; domain=" + domain + ";";
}
...
function delAutoCompleteCookie(name) {
  ...
  let domain = "";
  document.cookie = name + "=; path=/; expires=" + expire_date.toUTCString()
      + "; domain=" + domain + ";";
}
...
function addRcentSchList(sKeyword) {
  ...
  if (isRcntShow) {
    ...
    //과거날짜 중복 체크
    if (recentSch.indexOf(recentWord + delimiter) <= 0 && recentSch.indexOf(
        sKeyword + delimiter2) > 0) {   //현재날짜 중복체크
      let tot = parseInt(recentSch.indexOf(sKeyword + delimiter2)) + parseInt(
          delimiter2.length) + parseInt(sKeyword.length) + parseInt(
          dateForm.length) + parseInt(delimiter.length);
      let dup_data = recentSch.substring(
          recentSch.indexOf(sKeyword + delimiter2), tot);
      //console.log("dup_data:" + dup_data);
      recentSch = recentSch.replace(dup_data, "");
    }

    if (recentSch.indexOf(recentWord + delimiter) > 0) {    //현재날짜 중복체크
      recentSch = recentSch.replace(delimiter + recentWord + delimiter,
          delimiter);
    }
    ...
    if (recentSchCount > maxLength + 1) {
      ...
      recentSch = recentSch.substring(0,
          recentSch.lastIndexOf(delimiter) + delimiter.length);
    }
    let today = new Date();
    let expire_date = new Date(today.getTime() + 365 * 60 * 60 * 24 * 1000);
...
function getRcntKeywordList(recentSch) {
  ...
  for (let i = 0; i < recentWrdList.length - 1; i++) {
    ...
    addHtml += "                                              <a href='javascript:goSearchKeywordPage(`"
        + encodeURIComponent(recentWrd[0]) + "`)' class='recent_keyword_link'>";
    addHtml += "                                                  <span class='keyword'>"
        + escapetHtml(recentWrd[0]) + "</span>";
    addHtml += "                                              </a>";
    addHtml += "                                          </div>";
    addHtml += "                                          <div class='right_area'>";
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (let range of ageRange) {
        ...
        addHtml += "                                <option value='" + range
            + "' " + selected + ">" + range + upperChr + "</option>";
      }
      addHtml += "                                </select>";
      addHtml += "                            </div>";
      addHtml += "                            <!-- //form_sel -->"
      ...
      if (response.keywords && response.keywords.length > 0) {
        response.keywords.forEach(function (keyword, index) {
          let rankClass = (index < 3) ? "hot_keyword_item top"
              : "hot_keyword_item";
          let rankDisplay = (index < 9) ? "0" + (index + 1) : (index + 1);
          addHtml += "<li class='" + rankClass + "'>";
          addHtml += "  <div class='left_area'>";
          addHtml += "    <a href='javascript:goSearchKeywordPage(\""
              + encodeURIComponent(keyword) + "\")' class='hot_keyword_link'>";
        ...
        addHtml += "                     <p class='info_text font_size_xxs'>"
            + dateStr + " 기준</p>";
      }
      ...
      $("#ageSel-menu, #genderSel-menu").parent().addClass(
          "type_arw auto_complete_maintain");
    }
  });
}
...
function getRiseCallYmdStr(date) {
  ...
  return dateTemp[1].trim() + "." + dateTemp[2].trim() + " "
      + dateTemp[3].trim();
}
